...
class Agent(Default):
	_options = [
		Option('char', type=str, choices=characters.keys(),
			   help="character that this agent plays as"),
		Option('verbose', action="store_true", default=False,
			   help="print stuff while running"),
		Option('reload', type=int, default=60,
			   help="reload model every RELOAD seconds"),
	...
	def __init__(self, **kwargs):
		...
		self.datafile = h5py.File("statedata.hdf5", 'w')
		...
		self.history = util.CircularQueue(
			array=((self.rl.config.memory + 1) * ssbm.SimpleStateAction)())
		...
		if self.dump:
			...
			sock_addr = "tcp://%s:%d" % (self.trainer_ip,
										 util.port(self.rl.name + "/experience"))
			...
			address = "tcp://%s:%d" % (self.trainer_ip,
									   util.port(self.rl.name + "/params"))
		...
		if self.dump or self.disk:
			...
			self.dump_state_actions = (
				self.dump_size * ssbm.SimpleStateAction)()
	...
	def dump_state(self, state_action):
		...
		if self.dump_frame == self.dump_size:
			...
			if self.disk:
				path = os.path.join(
					self.dump_dir, self.dump_tag + '_%d' % self.dump_count)
	...
	def getstate(self, state, prev_action):
		print(state)
		f = self.flatten(state)
		# f.extend(prev_action.tolist())
		print(state)
		return f
	...
	def act(self, state, pad):
		...
		verbose = self.verbose and (
			self.action_counter % (10 * self.rl.config.fps) == 0)
		# verbose = False
		...
		#print(len(self.flatten(input_dict)))
		#print(current.state.players[0])
		# if verbose:
		...
		a = self.getAction(s)
		#print(a)
		self.rl.actionType.send(a, pad, self.char)
		# self.rl.actionType.send(self.action, pad, self.char) #this seems like the important bit
		# action is integer(for falcon falcon action type is old) char is fox,falco etc.
		'''
